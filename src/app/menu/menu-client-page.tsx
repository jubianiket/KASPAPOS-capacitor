...
export default function MenuClientPage({ initialMenuItems }: MenuClientPageProps) {
  // isLoading is now false by default, as initial data is pre-fetched
  ...
  const processAndSetCategories = (items: MenuItem[]) => {
     const uniqueCategories = ['All', ...Array.from(new Set(items.map(item => item.category).filter(Boolean) as string[]))];
     setCategories(uniqueCategories);
  }

  // Set initial categories from server-provided data
  useEffect(() => {
    processAndSetCategories(initialMenuItems);
  }, [initialMenuItems]);
  ...
  const handleFormSubmit = async (values: Partial<MenuItem>) => {
    try {
      ...
      fetchAndRefreshData(); // Refresh data after submit
    } catch (error) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setSelectedItem(null);
      fetchAndRefreshData(); // Refresh data after delete
    } catch (error) {
